            ...
            <div className="px-6 mt-8 space-y-10">
                ...
                {bookmarks.length > 0 && (
                    <section>
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <Link href="/library" className="text-[10px] font-bold uppercase tracking-widest text-tan-primary hover:text-brown-dark dark:hover:text-text-accent">Perpustakaan</Link>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {(() => {
                                ...
                                return (
                                    <Link href={`/novel/${hero.karya.id}/${hero.last_chapter}`} className="block group md:col-span-2">
                                        <div className="bg-white dark:bg-brown-dark/40 rounded-[2.5rem] p-5 border border-tan-light dark:border-brown-mid shadow-2xl flex flex-col sm:flex-row gap-6 group-active:scale-[0.98] transition-all relative overflow-hidden">
                                            ...
                                            <div className="w-28 sm:w-32 aspect-[3/4.2] rounded-2xl overflow-hidden shadow-2xl shrink-0 border border-tan-light/50 dark:border-brown-mid relative z-10">
                                                {hero.karya.cover_url ? <img src={hero.karya.cover_url} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" alt="" /> : <div className="w-full h-full bg-tan-light/20 flex items-center justify-center font-bold uppercase">{hero.karya.title}</div>}
                                        ...
                                        <div className="bg-white dark:bg-brown-dark/30 rounded-3xl p-4 border border-tan-light dark:border-brown-mid shadow-lg flex flex-col gap-4 group-active:scale-[0.98] transition-all relative border-l-4 border-l-tan-primary">
                                            <div className="flex gap-4 items-start">
                                                <div className="w-16 h-24 rounded-xl overflow-hidden shadow-md shrink-0 border border-tan-light/50 dark:border-brown-mid relative">
                                                    {b.karya.cover_url ? <img src={b.karya.cover_url} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" alt="" /> : <div className="w-full h-full bg-tan-light/10" />}
                    ...
                    <div className="flex gap-4 overflow-x-auto pb-4 hide-scrollbar scrollbar-hide">
                        {followedAuthorsRaw.length === 0 ? (
                            <div className="w-full py-8 px-6 bg-tan-light/20 rounded-[2.5rem] border border-dashed border-tan-light text-center">
                                ...
                                <Link href="/search" className="inline-block py-2 px-6 bg-tan-light/30 rounded-full text-[10px] font-black uppercase tracking-widest hover:bg-tan-light transition-all">Temukan Penulis</Link>
                            </div>
                        ) : (
                            followedAuthorsRaw.map((f: any) => (
                                <Link key={f.following.id} href={`/profile/${f.following.username}`} className="shrink-0 flex flex-col items-center gap-2 group">
                                    <div className="w-16 h-16 rounded-2xl overflow-hidden border-2 border-tan-light dark:border-brown-mid group-hover:border-tan-primary transition-all relative">
                                        {f.following.avatar_url ? <img src={f.following.avatar_url} className="w-full h-full object-cover" alt="" /> : <div className="w-full h-full bg-tan-light/20 flex items-center justify-center"><UserCircle2 className="w-8 h-8 text-tan-primary" /></div>}
                    ...
                    <section>
                        ...
                        <div className="flex gap-5 overflow-x-auto pb-6 hide-scrollbar scrollbar-hide">
                            {newWorksFollowed.map((f: any) => (
                                <Link key={f.id} href={`/novel/${f.id}`} className="shrink-0 w-44 flex flex-col gap-3 group">
                                    <div className="relative aspect-[3/4.2] w-full rounded-[2.5rem] overflow-hidden shadow-xl border border-tan-light/30 dark:border-brown-mid/40 transition-all duration-500">
                                        {f.cover_url ? <img src={f.cover_url} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" alt="" /> : <div className="w-full h-full bg-tan-light/20 flex items-center justify-center p-4 text-center text-xs text-tan-primary font-bold uppercase">{f.title}</div>}
                ...
                <section>
                    ...
                    <div className="flex gap-4 overflow-x-auto pb-6 hide-scrollbar scrollbar-hide">
                        {trending.map((f: any) => (
                            <Link key={f.id} href={`/novel/${f.id}`} className="shrink-0 w-36 flex flex-col gap-3 group">
                ...
                <section className="bg-tan-light/20 dark:bg-brown-mid/20 rounded-[3rem] p-6 border border-tan-light dark:border-brown-mid shadow-xl mb-12">
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <Link href="/library" className="w-9 h-9 rounded-full bg-tan-light/30 dark:bg-brown-mid/30 flex items-center justify-center text-tan-primary hover:bg-tan-primary hover:text-text-accent transition-all border border-tan-light"><History className="w-4 h-4" /></Link>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                        {bookmarks.slice(0, 8).map(b => (
                            <Link key={b.id} href={`/novel/${b.karya.id}`} className="flex items-center gap-3 p-3 bg-white/50 dark:bg-brown-dark/20 rounded-2xl hover:bg-white dark:hover:bg-brown-mid/40 border border-tan-light/30 dark:border-brown-mid transition-all group overflow-hidden relative shadow-sm hover:shadow-md border-l-4 border-l-tan-primary">
                                <div className="w-12 h-16 rounded-xl overflow-hidden shrink-0 shadow-sm border border-tan-light/50 dark:border-brown-mid">{b.karya.cover_url ? <img src={b.karya.cover_url} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" alt="" /> : <div className="w-full h-full bg-tan-light/10" />}</div>
