...
import { Star, Eye, BookOpen, Heart, UserCircle2, Users, Check, ChevronRight, History, Flame } from "lucide-react";
                        ...
                        {newWorksFollowed.map((f: any) => (
                            <Link key={f.id} href={`/novel/${f.id}`} prefetch={false} className="shrink-0 w-44 flex flex-col gap-3 group">
                                <div className="relative aspect-[3/4.2] w-full rounded-[2.5rem] overflow-hidden shadow-xl border border-tan-light/30 dark:border-brown-mid/40 transition-all duration-500">
                                    {f.cover_url ? <Image src={f.cover_url} width={176} height={246} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" alt="" /> : <div className="w-full h-full bg-tan-light/20 flex items-center justify-center p-4 text-center text-xs text-tan-primary font-bold uppercase">{f.title}</div>}
                    ...
                    {trending.map((f: any) => (
                        <Link key={f.id} href={`/novel/${f.id}`} prefetch={false} className="shrink-0 w-36 flex flex-col gap-3 group">
                            <div className="relative aspect-[3/4.2] w-full rounded-[2.5rem] overflow-hidden shadow-lg border border-tan-light/30 dark:border-brown-mid/40">
                                {f.cover_url ? <Image src={f.cover_url} width={144} height={200} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" alt="" /> : <div className="w-full h-full bg-tan-light/20 flex items-center justify-center p-4 text-center text-xs text-tan-primary font-bold uppercase">{f.title}</div>}
                                <div className="absolute top-3 inset-x-3 flex justify-between items-start">
