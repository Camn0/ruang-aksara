...
export default function CommentItem({
    ...
    return (
        <div className={`relative ${visualDepth > 0 ? 'ml-4 sm:ml-6 mt-4' : 'mb-8'}`}>
            ...
            <div className="flex gap-3 items-start relative bg-white dark:bg-slate-900 z-10">
                {/* Voting Tray - Reddit Style Sidebar */}
                <div className="flex flex-col items-center gap-1 min-w-[32px] pt-1 sticky top-24">
                    <button
                        onClick={() => handleVote(1)}
                        className={`p-1 rounded-lg transition-all active:scale-125 ${comment.userVote === 1 ? 'text-orange-500 bg-orange-50 dark:bg-orange-950/30 ring-2 ring-orange-200 dark:ring-orange-900 shadow-sm shadow-orange-100 dark:shadow-none' : 'text-gray-400 hover:text-orange-500 hover:bg-orange-50 dark:hover:bg-orange-950/20'}`}
                    >
                        <ChevronUp className={`w-5 h-5 ${comment.userVote === 1 ? 'fill-current stroke-[3]' : ''}`} />
                    </button>
                    <span className={`text-[11px] font-black ${comment.userVote === 1 ? 'text-orange-600' : comment.userVote === -1 ? 'text-indigo-600' : 'text-gray-600 dark:text-gray-400'}`}>
                        {comment.score}
                    </span>
                    <button
                        onClick={() => handleVote(-1)}
                        className={`p-1 rounded-lg transition-all active:scale-125 ${comment.userVote === -1 ? 'text-indigo-600 bg-indigo-50 dark:bg-indigo-900/30 ring-2 ring-indigo-200 dark:ring-indigo-900 shadow-sm shadow-indigo-100 dark:shadow-none' : 'text-gray-400 hover:text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-950/20'}`}
                    >
