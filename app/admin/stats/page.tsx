...
const STATS_OVERVIEW_CONFIG = [
    ...
    },
    {
        ...
        textColor: "text-[#3B2A22] dark:text-bg-cream",
        ...
        <div className="min-h-screen bg-bg-cream/60 dark:bg-brown-dark transition-colors duration-500 pb-24">
            <div className="max-w-6xl mx-auto px-6 pt-12">
                <Link 
                    ...
                    className="inline-flex items-center gap-2 text-[10px] font-black text-text-main/50 dark:text-tan-light uppercase tracking-[0.3em] hover:text-text-main dark:hover:text-bg-cream mb-8 transition-colors group"
                >
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12">
                    <div>
                        ...
                        <p className="text-text-main/50 dark:text-tan-light font-medium italic">Pusat data untuk melacak pertumbuhan dan performa karya Anda di Ruang Aksara.</p>
                    </div>
                    ...
                    <div className="bg-white/40 dark:bg-brown-mid/20 rounded-[2rem] p-6 border border-text-main/5 dark:border-white/5 flex items-center gap-6 group transition-all">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-[9px] font-black uppercase tracking-widest text-text-main/50 dark:text-tan-light">Total Followers</p>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <p className="text-[7px] font-black uppercase tracking-widest text-text-main/50 dark:text-tan-light">MINGGU INI</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-16">
                    <div className="lg:col-span-2 bg-white/40 dark:bg-brown-dark/40 rounded-[3rem] p-10 border border-text-main/5 dark:border-white/5 shadow-inner">
                        <div className="grid md:grid-cols-2 gap-12">
                            <Link href="/admin/stats/karya" className="group/chart relative">
                                ...
                                <h3 className="text-[11px] font-black uppercase tracking-[0.3em] text-text-main/50 dark:text-tan-light mb-8 flex items-center gap-2">
                                    <Clock className="w-4 h-4" /> Activity Map (Last 30d)
                                </h3>
                                <ActivityBarChart data={activityMapData} />
                                <div className="mt-8 flex justify-between items-center text-[10px] font-black uppercase tracking-widest italic text-text-main/50 dark:text-tan-light">
                                    <span>H-30</span>
                ...
                <div className="flex items-center gap-6 mb-8">
                    ...
                    <div className="flex-1 h-[1px] bg-text-main/10 dark:bg-white/10"></div>
                </div>
                        ...
                        >
                            ...
                            <div className="relative z-10 flex items-end justify-between">
                                <div>
                                    ...
                                    <p className="text-[10px] font-black uppercase tracking-[0.3em] text-white/60 dark:text-tan-light group-hover:opacity-100 transition-opacity">{conf.label}</p>
                                </div>
                                <div className="flex flex-col items-end gap-2">
                                    ...
                                    <span className="text-[7px] font-black uppercase tracking-widest text-white/60 dark:text-tan-light opacity-0 group-hover:opacity-100 transition-opacity">Lihat Analisis</span>
                                </div>
                            </div>
    ...
    return (
        <div className="flex items-end gap-2 w-full h-24 pt-4 group">
            {data.map((val, i) => (
                <div key={i} className="flex-1 relative group/bar h-full flex items-end">
                    {/* Ghost Bar */}
                    <div className="absolute inset-x-0 bottom-0 h-[3px] bg-text-main/5 dark:bg-white/5 rounded-t-sm" />
                    <div 
...
function SentimentBreakdown({ percentage, total, distribution, showHint }: { percentage: number, total: number, distribution?: { score: number, count: number }[], showHint?: boolean }) {
    return (
        <div className="w-full bg-white/40 dark:bg-brown-mid/10 rounded-[3rem] p-8 border border-white/5 shadow-inner relative group/gauge transition-all group-hover/card:bg-white/60 dark:group-hover/card:bg-brown-mid/20">
            {showHint && (
            ...
            <div className="flex justify-between items-end mb-8">
                <div>
                    ...
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-text-main/50 dark:text-tan-light mt-2 italic">Global Sentiment</p>
                </div>
                        ...
                        <div key={d.score} className="space-y-1">
                            <div className="flex justify-between items-center px-1">
                                <div className="flex items-center gap-1">
                                    <span className="text-[9px] font-black tracking-tighter text-text-main/50 dark:text-tan-light">{d.score} Star</span>
                                    <Star className={`w-2 h-2 ${d.score >= 4 ? 'text-tan-primary fill-tan-primary/80' : 'text-text-main/10 dark:text-white/10'}`} />
                                </div>
                                <span className="text-[8px] font-black text-text-main/40 dark:text-tan-light uppercase tracking-tighter">{d.count}</span>
                            </div>
