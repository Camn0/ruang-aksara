...
export default function EditKaryaForm({ karya, allGenres, children }: { karya: Karya, allGenres: Genre[], children: React.ReactNode }) {
    ...
    const [coverBase64, setCoverBase64] = useState<string | null>(karya.cover_url); // Menyimpan format text URL
    const fileInputRef = useRef<HTMLInputElement>(null);

    const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const file = e.target.files?.[0];
        if (file) {
            // 1. Tampilkan Preview untuk mata user
            setCoverPreview(URL.createObjectURL(file));

            // 2. Ubah file jadi teks Base64 agar bisa disimpan ke database tanpa backend khusus
