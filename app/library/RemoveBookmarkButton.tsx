...
export default function RemoveBookmarkButton({ karyaId, onRemoved }: { karyaId: string, onRemoved?: () => void }) {
    ...
    return (
        <button
            ...
            disabled={removing}
            className="absolute top-0 right-0 z-20 w-8 h-8 bg-paper wobbly-border border-2 border-ink/10 text-ink/20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all hover:bg-dried-red hover:text-parchment hover:border-dried-red active:scale-90 rotate-6"
